...
int main(int argc, char **argv)
{
	...
	// Correctness
	double correctness = (double)numcorrect / num_tracefiles;
	correctness *= 30;

	// Performance
	double performance = perfindex / 100;
	performance *= 70;

	// Final score
	double final_score = correctness + performance;

	printf("--------------------------------------------\n");
	printf("Correctness(30%) : %d / %d \t-> %.0f points\n", numcorrect, num_tracefiles, correctness);
	printf("Performance(70%) : %.0f / 100 \t-> %.0f points\n", perfindex, performance);
	printf("Final Score :  %.0f (correctness) + %.0f (performance) = %.0f / 100\n", correctness, performance, final_score);
	printf("--------------------------------------------\n");
    }
